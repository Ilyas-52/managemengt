...
export default function HamzaAttendance({ students, attendanceData, toggleLesson }: Props) {
    ...
    // 🕵️‍♂️ دالة مساعدة ذكية: النهار من يوسف والساعة من السيرفر (بيطون)
    const formatRegDate = (regDate?: string, createdAt?: string) => {
        if (!regDate) return null;

        // النهار والشهر والسنة من تاريخ يوسف
        const datePart = new Date(regDate).toLocaleDateString('fr-FR', {
            day: '2-digit',
            month: '2-digit',
            year: 'numeric'
        });

        // الساعة والدقيقة من created_at ديال السيرفر
                        ...
                        <div key={student.id} className="bg-white border-2 border-slate-100 rounded-[30px] p-6 shadow-sm">
                            <div className="flex justify-between items-start mb-4 border-b border-slate-50 pb-3">
                                <div className="flex flex-col gap-1">
                                    ...
                                    {/* 🕒 مسمار التاريخ والوقت فالتليفون */}
                                    {student.registration_date && (
                                        <div className="flex items-center gap-1 text-[9px] text-slate-400 font-bold">
                                            <Clock size={10} />
                                            <span>{formatRegDate(student.registration_date, student.created_at)}</span>
                                        </div>
                                    )}
                                </div>
                                    ...
                                    <tr key={student.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="py-4 px-6 sticky right-0 z-20 bg-white border-l-2 border-slate-100 shadow-[2px_0_5px_rgba(0,0,0,0,02)]">
                                            <div className="flex flex-col">
                                                ...
                                                {/* 🕒 مسمار التاريخ والوقت فـ الـ PC */}
                                                {student.registration_date && (
                                                    <span className="text-[8px] text-slate-400 font-bold flex items-center gap-1 mt-1">
                                                        <Clock size={8} />
                                                        {formatRegDate(student.registration_date, student.created_at)}
                                                    </span>
                                                )}
                                            </div>
                                        </td>
